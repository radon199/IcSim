...
SN74HC163N::SN74HC163N(std::string name) : ChipBase(name)
{
    // Init pins
    PinMap pins_ = {
        {"MR",  1},
...
SN74HC163N::cook(bool& prop)
{
    ...
    if (clk && !clocked_) {
        ...
        if (get_input("PE") == 0) {
            const int b0 = get_input("D0");
            set_data("Q0", b0, prop);
            const int b1 = get_input("D1");
            set_data("Q1", b1, prop);
            const int b2 = get_input("D2");
            set_data("Q2", b2, prop);
            const int b3 = get_input("D3");
            set_data("Q3", b3, prop);
            set_data("TC", 0, prop);
            std::bitset<4> count;
            count[0] = b0;
